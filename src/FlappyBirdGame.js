...
export default function FlappyBirdGame() {
  ...
  useEffect(() => {
    ...
    const draw = now => {
        ...
        for (const pipe of game.pipes) {
          if (!pipe.passed && pipe.x + PIPE_W < BIRD_X) {
            ...
            if (game.score > bestRef.current) {
              bestRef.current = game.score; setBest(game.score); saveBest(game.score);
            }
          }
      ...
      game.trail = game.trail.filter(dot => dot.life > 0);
      game.trail.forEach(dot => { dot.x -= 1.2 * dt; dot.life -= 0.055 * dt; });
      game.trail.forEach(dot => {
        ctx.globalAlpha = Math.max(0, dot.life * 0.55); ctx.fillStyle = "#4CE0FF";
        ctx.beginPath(); ctx.arc(dot.x, dot.y, 3 * dot.life, 0, Math.PI * 2); ctx.fill();
      }); ctx.globalAlpha = 1;
  ...
  return <div className="glass-card flappy-card">
    ...
    <div className="game-hint">Tap, click, or press <b>SPACE</b> / <b>↑</b> to flap · Your best score is saved on this device.</div>
