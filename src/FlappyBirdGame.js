...
export default function FlappyBirdGame() {
  ...
  const gameRef = useRef({ birdY: HEIGHT / 2, velocity: 0, pipes: [], score: 0, started: false, over: false, last: 0, speed: 2.7 });
  ...
  const reset = () => {
    gameRef.current = { birdY: HEIGHT / 2, velocity: 0, pipes: [{ x: WIDTH + 80, gapY: 145 + Math.random() * 75, passed: false }], score: 0, started: true, over: false, last: performance.now(), speed: 2.7 };
  ...
  useEffect(() => {
    ...
    const draw = (now) => {
      ...
      // Futuristic flight-grid and moving scanline.
      ...
      const scan = (now * 0.035) % HEIGHT;
      ctx.fillStyle = "rgba(76,224,255,.035)"; ctx.fillRect(0, scan, WIDTH, 2);

      if (game.started && !game.over) {
        game.velocity += GRAVITY * dt;
        game.birdY += game.velocity * dt;
        game.speed = Math.min(5.4, 2.7 + game.score * 0.06);
        game.pipes.forEach(pipe => { pipe.x -= game.speed * dt; });
        if (game.pipes.length === 0 || game.pipes[game.pipes.length - 1].x < WIDTH - 250) {
          game.pipes.push({ x: WIDTH + 35, gapY: 105 + Math.random() * 120, passed: false });
        }
        game.pipes = game.pipes.filter(pipe => pipe.x > -PIPE_W - 20);

        for (const pipe of game.pipes) {
          if (!pipe.passed && pipe.x + PIPE_W < BIRD_X) {
            pipe.passed = true; game.score += 1; setScore(game.score);
            if (game.score > bestRef.current) { bestRef.current = game.score; setBest(game.score); }
          }
      ...
      for (const pipe of game.pipes) {
        ...
        const drawPipe = (x, y, h, capY) => {
          ...
          ctx.fillStyle = grad; ctx.shadowBlur = 15; ctx.shadowColor = "#4CE0FF"; ctx.fillRect(x, y, PIPE_W, h);
          ctx.fillRect(x - 5, capY, PIPE_W + 10, 13); ctx.shadowBlur = 0;
          ctx.fillStyle = "rgba(5,7,13,.16)"; ctx.fillRect(x + 9, y, 6, h);
        };
        drawPipe(pipe.x, 0, topH, topH - 13);
        drawPipe(pipe.x, bottomY, HEIGHT - bottomY, bottomY);
      }
      ...
      ctx.fillStyle = "#8A5CFF"; ctx.beginPath(); ctx.moveTo(12, 1); ctx.lineTo(24, 5); ctx.lineTo(12, 8); ctx.closePath(); ctx.fill();
      ctx.restore();

      if (!game.started || game.over) {
        ctx.fillStyle = "rgba(5,7,13,.63)"; ctx.fillRect(0, 0, WIDTH, HEIGHT); ctx.textAlign = "center";
        ctx.fillStyle = "#B9FBFF"; ctx.font = "800 30px Inter, system-ui, sans-serif"; ctx.fillText(game.over ? "RUN TERMINATED" : "NEURAL FLIGHT", WIDTH / 2, HEIGHT / 2 - 22);
        ctx.fillStyle = "#9AA4B2"; ctx.font = "500 12px Inter, system-ui, sans-serif"; ctx.fillText(game.over ? `Score ${game.score} · Tap or press SPACE to restart` : "Tap / click / SPACE to flap", WIDTH / 2, HEIGHT / 2 + 12);
      }
  ...
  return <div className="glass-card flappy-card">
